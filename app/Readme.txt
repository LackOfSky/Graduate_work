...
Впровадити медіасервер в проект.
 ...
 (to check)  таск 44. Создать папку для хранения медиа сообщений.
 ...
 (to check) таск 55. Передача медіа userInfo   (class ChangesNotifierUseCase)
 (to check) таск 14. відправка баннеру користувача окремим send media request. при цьому server handler має назначати URI в userInfo при збереженні зображення
            (також розбити на два окремі оновлення: about та banner
 (to check) таск 61. nettyClientHandler открывать клиент для передачи медиа.
 ...
 таск (указать номер). присваивать медиасерверу адрес с НСД
...next 66.
