...
v0.15.2
Впровадити медіасервер в проект.
 (done) таск 62. Медіа-повідомлення items. Відображати разом з повідомленням. пофіксити їх відображення при перезаході в додаток   ChatDialogViewModel
            (ми фактично не зберігали зображення окремо в папку нашого застосунку) (media external)
 (stashed)таск 60. спроектувати ResponseData та впровадити в хендлери
 (done) таск 60. в нетти сервер написать обработчик MessageType.REQUEST_MEDIA_SERVER,
 ...
 (done) таск 69. присваивать медиасерверу адрес канала через клиента
 (done) таск 11. отправка медиа через чат, возможность просмотра фото в чате
 (done) таск 61. nettyClientHandler открывать клиент для передачи медиа.
 (done) таск 55. Передача медіа userInfo   (class ChangesNotifierUseCase)
 (done)  таск 44. Создать папку для хранения медиа сообщений.
 ...
 (done) таск 14. відправка баннеру користувача окремим send media request. при цьому server handler має назначати URI в userInfo при збереженні зображення
            (також розбити на два окремі оновлення: about та banner

v0.15.3
 (done) таск 74. баг клиента (при удалении клиента "только у себя", и второй нажал "удалить у всех" - клиент что нажал - отлетел
            (На данному етапі, якщо товариш онлайн - зробити кнопку "видалити лише в себе - не активною", коли оффлайн - не активна "видалити у всіх")
 (done) таск 78. корректное отображение Прикреплённых к отправке файлов соответствующих типов. #//TODO TODODODO
                (+- чорновий варіант виконаний)
 (done) таск 65. можливість вибору файлу з файлової системи
         (поки - через type document, далі - буде видно)
 (done) таск 63. інтерфейс для прикріплення різних типів файлів в повідомлення
             відображення різних типів файлів в чаті та їх розподілення
 (криво, але done) таск 64. можливість відтворення аудио та відео в чаті \ відкривання їх через intend

 v0.15.4
 (abandoned) таск 48. чи треба відкривати mediaHandler (media server) в скопі корутин.
           Також щодо MediaChunk та як він реалізовує запис у файл.
 (abandoned) таск 52. media handler зробити обробник помилок для chat inactive
 (done) таск 70. отправка медиа через чат, возможность просмотра видео в чате
 (done) таск 71. отправка медиа через чат, возможность просмотра аудио в чате
 (done) таск 79. фікс кнопки "пауза", що закриває огляд самого відео в відео карточці (повідомлення)
 (done) таск 80. додати строку з відображенням поточного моменту відтворення для аудіо
 (done) таск 25. в  Message сделать поле "reply" default null, при нажатии туда - подкреплять сообщение к полю ввода
             ( с крестиком для возможности закрытия)

 v0.15.5
 (done) таск 75. однобокое добавление userInfo. добавляет только тот, кто approve запрос
 (done) таск 57. FriendsScreen, ChatsScreen фікс іконок
 (done) таск 40. при изменении UserInfo  (about)  крашнулся принимающий клиент
 (done) таск 77. фікс FriendProfile (додати відображення about та info)
 (done) таск 86. при изменении UserInfo  (about)  у "друга" обнулились лого та баннер
                    (виглядає, як фактично "вибиває" старе userInfo, та додає з пустими значеннями)

 v0.15.6
 (done) таск 76. проверить как приходит Logo. (через профиль, или друзей)
 (done) таск 82. message dialog item картка відображення повідомлення - якщо в повідомленні є reply message id, то ссилатися на те повідомлення

 v0.16
 (done) таск 73. помилка надсилання медіа-лого. вказує данні файлу, що надсилались в перший раз
            (перевірити поведінку серверу),
            (перевірити поведінку notifier-a)  (якщо баг продовжиться - прологувати sendMediaLogo) (з виду має працювати корректно)
            (Перевірити чи буде зберігатись помилка, при перезапуску серверу, а потім при перезапуску лише клієнту)
            Перевірити Log.d(TAG, "sendUserLogoFile fileDetails.name: ${fileDetails.name}")
            Костыли в юзер инфо (clientPartP2P + profile)
 (done) таск 84. баг оновлення зображення логотипу друга. оновлюється лише один раз при коннекті до друга.
            а при перезапуску служби встановлює попереднє фото, що було (оновлюємо на геть нове, а ставить друге, проміжне)
            (виглядає як: спочатку відсилає зображення, потім його встановлює у себе)
            (Хоча з вигляду, від профілю до нетті-клієнту все гаразд)
 (done) таск 72. відладити mediaServer. (Безпосередньо передачу медіа)

 v0.20
 (done) таск 81. reply message. зміщувати список до потрібного повідомлення коли натискаєш на прикріплене reply, та кнопка пролистати вниз
 (done) таск 92. помилка доступу до аудіофайлу viewModel.getAudioMetadata(context, uri)
                (була проблема оновлення урі (допомогло collectAsState)
 (done) таск 83. передача медіа-повідомлень між користувачами.
                транспортна логіка.
                (за спрощеною логікою - якщо має бути медіа - в інтерфейсі створити плейсхолдер "медіа не існує\неможливо відтворити"
                (паралельно надсилати медіа за процедурою, як з логотипом\баннером, коли медіа сервер повністю завантажить - оновлює конкретне повідомлення, встановлюючи uri на файл)
                (перевірити працездатність передачі медіа)
 (done) таск 94. помилка відео. потребує оновлення екрану, хоча урі повідомлення має перемальовувати відео при завантаженні

 v0.20.1



 таск 95. проблема пересылки файлов большого обьема (видео больше 50 мб пока успешно не отправлялось)
                (переповнюється буфер при надсиланні (NettyMediaClient)

 таск 87. Костыли в юзер инфо (clientPartP2P + profile)

<<<<
 таск 85. динамічно не перевіряє відповідність даних друга при підключенні
            (надсилати userInfo, перевевіряти за полями, якщо знайдена невідповідність - кидати
                    запит надсилаючому, на оновлення (інфо, про користувача, баннер, зображення)
 таск 89. рефакторинг надсилання оновлених userInfo. При зміні userInfo надсилати запит іншому клієнту - звіряти данні, якщо відрізняються - то надсилати те, що йому треба
>>>>

 таск 91. медіа повідомлення надсилаються, в файловій системі відображаються, але не відображаються безпосередньо в повідомленнях чату
            (іноді відображаються)
 таск 93. "проскользуючі" повідомлення. Опис: декілька разів надсилаєш медіа повідомлення, потім другий клієнт надсилає - і воно не доходить до нас
                service Gr...ia handler com.lackofsky.cloud_s   Error parsing message. non typical message: ""}{"content":"{\"messageId\":\"f51f71ead1ea936c94\",\"requestedIntend\":\"MEDIA_EXTERNAL\",\"userUniqueId\":\"f8d247f2e88f7388\"}","messageType":"REQUEST_MEDIA_SERVER","ownServerPort":0,"sender":"{\"fullName\":\"xiao\",\"id\":1,\"ipAddr\":\"\",\"login\":\"mi\",\"port\":0,\"uniqueID\":\"f8d247f2e88f7388\"}","senderId":"f8d247f2e88f7388","senderIp":""}. Error: com.google.gson.JsonSyntaxException: java.lang.IllegalStateException: Expected BEGIN_OBJECT but was STRING at line 1 column 2 path $




 таск 16. додати можливість скидати фото профілю (баннер, логотип) ( видаляти)
 ...
 таск 53. TransferMediaIntend.MEDIA_USER_LOGO -> отримувати лого з URI / чи з файлу,додавати в бд та видаляти лого з пам'яті пристрою
 ...
 таск 59. отчёт сервера клиенту за каждый принятый пакет
            должно быть динамическое отображение скачаного файла каждую секунду
                (сколько загружено \ или сколько осталось \ так же можна указывать скорость загрузки)
 таск 66. збереження медіа: логіка перевірки чи файл не повторюється (якщо файл з тим же ім'ям існує - перевірити його об'єм
            в пам'яті (якщо різний, то створити новий об'єкт, якщо однаковий - замінити\залишити як є)
            (актуально лише для медіа з повідомлень)
 таск 67*. clean architecture - отримувати в класи інтерфейси, а через хілт прокидувати потрібні класи

 таск 68*. покрити класи тестами

 таск 88. надсилати медіа до повідомлення за message id, якщо данний id знаходиться в Пулі

 таск 90. delete for all критична помилка, коли в одного клієнта друга немає, а інший його видаляє з друзів
...
...next 96.
 ...
 Концепція 5. клієнт для медіа. Розбивати медіа на шматки вказаної довжини. якщо сервер втратить певний шматок, то звернеться до клієнта, щоб він переслав його заново
            val chunks = mutableListOf<String>()
                --> Створити архетип клієнту p2p, щоб відслідковувати
                                                                           передані шматки даних, (щоб візуально виводити на екран відсотки)
                                                                           чергу на передачу медіа
 ...
 Концепція 7. подумати щодо використання protobuf

 Концепція 8. Окрема сутність connection: тримає в собі активних клієнтів та посилання на їх сервер, посилання на медіа-сервер
                            (якщо медіа-сервер не доступний для підключення - звертатись на сервер для оновлення даних (порта підключення) )
------------------------------------------------------------------------------------------------------------
